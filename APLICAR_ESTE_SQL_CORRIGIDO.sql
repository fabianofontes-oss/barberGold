-- =====================================================
-- BARBERGOLD - SQL CORRIGIDO (SEM ERRO DE VIEW)
-- Data: 2025-01-28
-- =====================================================
-- Este SQL corrige o erro: "cannot drop columns from view"
-- =====================================================

-- =====================================================
-- PASSO 1: DESCOBRIR E DROPAR VIEWS QUE DEPENDEM DE PROFILES
-- =====================================================

-- Dropar view tenants_registry se existir (ela pode depender de profiles)
DROP VIEW IF EXISTS tenants_registry CASCADE;

-- Dropar qualquer outra view que possa estar interferindo
DROP VIEW IF EXISTS stores CASCADE;

-- =====================================================
-- PASSO 2: ADICIONAR COLUNA store_id NA TABELA PROFILES
-- (Agora sem conflito de VIEWs)
-- =====================================================
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE NOTICE '✅ Coluna store_id adicionada à tabela profiles';
  ELSE
    RAISE NOTICE 'ℹ️ Coluna store_id já existe na tabela profiles';
  END IF;
END $$;
...
BEGIN
  -- Se tenant_id foi preenchido, copiar para store_id
  IF NEW.tenant_id IS NOT NULL THEN
    NEW.store_id := NEW.tenant_id;
  END IF;
  -- Se store_id foi preenchido, copiar para tenant_id
...
-- =====================================================
-- PASSO 4: RECRIAR VIEW stores APONTANDO PARA tenants
-- =====================================================
...
RAISE NOTICE '✅ VIEW stores criada';

-- =====================================================
-- PASSO 5: RECRIAR VIEW tenants_registry (SE NECESSÁRIO)
-- =====================================================

CREATE OR REPLACE VIEW tenants_registry AS
SELECT
  id,
  name,
  slug,
...
COMMENT ON VIEW tenants_registry IS 'VIEW de compatibilidade para código legado';

RAISE NOTICE '✅ VIEW tenants_registry recriada';

-- =====================================================
-- PASSO 6: CRIAR TABELA staff_payments
-- =====================================================
...
RAISE NOTICE '✅ Tabela staff_payments criada';

-- =====================================================
-- PASSO 7: ATUALIZAR POLICIES DAS TABELAS EXISTENTES
-- =====================================================
...
-- =====================================================
-- PASSO 8: ATUALIZAR TRIGGER DE SIGNUP
-- =====================================================
...
BEGIN
    ...
    -- Criar profile com tenant_id E store_id (sincronizados)
...
RAISE NOTICE '✅ Trigger de SignUp atualizado';

-- =====================================================
-- FIM - SUCESSO!
-- =====================================================
-- ✅ VIEWs dropadas e recriadas sem conflito
