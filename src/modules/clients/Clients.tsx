    ...
    <div className="h-full flex flex-col animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 pb-20">
        {filteredClients.length === 0 && (
          <div className="col-span-full py-16 flex flex-col items-center justify-center text-zinc-500 animate-fade-in">
             {searchQuery ? (
                <>
                   <Search className="w-16 h-16 mb-4 opacity-20" />
                   <p className="text-xl font-bold mb-1 text-zinc-400">{t('clients.noResults')}</p>
                   <p className="text-sm text-zinc-600">Try searching for a different name or phone number</p>
                </>
             ) : (
                <>
                   <Users className="w-16 h-16 mb-4 opacity-20" />
                   <p className="text-xl font-bold mb-4 text-zinc-400">{t('clients.noClients')}</p>
                   <button
                     onClick={() => setIsModalOpen(true)}
                     className="bg-zinc-800 hover:bg-zinc-700 text-amber-500 font-bold py-2.5 px-6 rounded-xl flex items-center gap-2 transition-all"
                   >
                     <UserPlus className="w-5 h-5" /> {t('clients.addFirst')}
                   </button>
                </>
             )}
          </div>
          ...
          <div className="bg-zinc-900 w-full h-full md:h-auto md:max-w-md md:rounded-2xl border-0 md:border border-zinc-800 p-6 shadow-2xl overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-6 md:space-y-4">
              ...
              <div className="bg-zinc-950/50 p-4 rounded-xl border border-zinc-800">
                 <label htmlFor="dep-name" className="block text-xs font-bold text-white mb-2 uppercase flex items-center gap-2"><Users className="w-3 h-3 text-amber-500" /> Dependents (Family)</label>
                 <div className="flex gap-2 mb-2">
                    <input id="dep-name" type="text" placeholder="Name (e.g. Son)" value={newDependentName} onChange={e => setNewDependentName(e.target.value)} className="flex-1 bg-zinc-900 border border-zinc-700 rounded-lg px-2 py-1.5 text-sm text-white focus:border-amber-500 outline-none" />
