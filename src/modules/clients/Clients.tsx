...
export const Clients = () => {
  ...
  return (
    <div className="h-full flex flex-col animate-fade-in">
      ...
      {/* Add Client Modal */}
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-900 md:bg-black/70 md:backdrop-blur-sm p-0 md:p-4" role="dialog" aria-modal="true" aria-labelledby="modal-add-client-title">
          <div className="bg-zinc-900 w-full h-full md:h-auto md:max-w-md md:rounded-2xl border-0 md:border border-zinc-800 p-6 shadow-2xl overflow-y-auto">
            <div className="flex justify-between items-center mb-6">
               <h3 id="modal-add-client-title" className="text-xl font-bold text-white">Add New Client</h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-6 md:space-y-4">
              ...
              <div className="bg-zinc-950/50 p-4 rounded-xl border border-zinc-800">
                 ...
                 <div className="flex gap-2 mb-2">
                    <input type="text" aria-label="New dependent name" placeholder="Name (e.g. Son)" value={newDependentName} onChange={e => setNewDependentName(e.target.value)} className="flex-1 bg-zinc-900 border border-zinc-700 rounded-lg px-2 py-1.5 text-sm text-white focus:border-amber-500 outline-none" />
                 ...
                 <div className="space-y-2">
                    {formData.dependents.map((dep, idx) => (
                       <div key={idx} className="flex justify-between items-center text-sm bg-zinc-900 p-2 rounded border border-zinc-800">
                          ...
                          <button type="button" aria-label={`Remove dependent ${dep.name}`} onClick={() => removeDependent(dep.id)} className="text-red-500 hover:text-red-400"><X className="w-3 h-3" /></button>
                       </div>
                    ))}
                 </div>
              </div>
      ...
      {selectedClient && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-900 md:bg-black/70 md:backdrop-blur-sm p-0 md:p-4" role="dialog" aria-modal="true" aria-labelledby="modal-client-detail-title">
              ...
              <div className="p-6 border-b border-zinc-800 flex justify-between items-start bg-zinc-950/50">
                 <div className="flex gap-4">
                    ...
                    <div className="flex-1">
                       <h2 id="modal-client-detail-title" className="text-2xl font-bold text-white">{selectedClient.name}</h2>
                       ...
                       <div className="space-y-2">
                          ...
                          {(selectedClient.dependents || []).length === 0 ? <p className="text-zinc-500 text-sm italic">No dependents listed.</p> : (selectedClient.dependents || []).map(dep => (
                             <div key={dep.id} className="flex justify-between items-center bg-zinc-950 p-3 rounded-lg border border-zinc-800">
                                ...
                                <button onClick={() => removeDependent(dep.id)} aria-label={`Remove dependent ${dep.name}`} className="text-zinc-600 hover:text-red-500"><Trash2 className="w-4 h-4" /></button>
                             </div>
                          ))}
                       </div>
                    </div>
