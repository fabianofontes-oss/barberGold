    ...
    <div className="h-full flex flex-col animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 pb-20">
        {filteredClients.length === 0 && (
          <div className="col-span-full py-16 flex flex-col items-center justify-center text-zinc-500/50">
            <Users className="w-16 h-16 mb-4 stroke-1" />
            <p className="text-lg font-medium">{searchQuery ? t('clients.noResults') : t('clients.noClients')}</p>
            {searchQuery && (
              <button
                onClick={() => setSearchQuery('')}
                className="mt-2 text-amber-500 hover:text-amber-400 text-sm hover:underline"
              >
                Clear search
              </button>
            )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-900 md:bg-black/70 md:backdrop-blur-sm p-0 md:p-4">
          <div className="bg-zinc-900 w-full h-full md:h-auto md:max-w-md md:rounded-2xl border-0 md:border border-zinc-800 p-6 shadow-2xl overflow-y-auto">
            <div className="flex justify-between items-center mb-6">
               ...
               <button onClick={() => setIsModalOpen(false)} aria-label="Close" className="bg-zinc-800 p-2 rounded-full text-zinc-400 hover:text-white"><X className="w-5 h-5"/></button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6 md:space-y-4">
              ...
              <div className="bg-zinc-950/50 p-4 rounded-xl border border-zinc-800">
                 ...
                 <div className="flex gap-2 mb-2">
                    <input aria-label="New dependent name" type="text" placeholder="Name (e.g. Son)" value={newDependentName} onChange={e => setNewDependentName(e.target.value)} className="flex-1 bg-zinc-900 border border-zinc-700 rounded-lg px-2 py-1.5 text-sm text-white focus:border-amber-500 outline-none" />
                 ...
                 <div className="space-y-2">
                    {formData.dependents.map((dep, idx) => (
                       <div key={idx} className="flex justify-between items-center text-sm bg-zinc-900 p-2 rounded border border-zinc-800">
                          ...
                          <button type="button" onClick={() => removeDependent(dep.id)} aria-label={`Remove ${dep.name}`} className="text-red-500 hover:text-red-400"><X className="w-3 h-3" /></button>
                       </div>
                    ))}
                 </div>
              </div>
                       ...
                       <div className="space-y-2">
                          ...
                          {(selectedClient.dependents || []).length === 0 ? <p className="text-zinc-500 text-sm italic">No dependents listed.</p> : (selectedClient.dependents || []).map(dep => (
                             <div key={dep.id} className="flex justify-between items-center bg-zinc-950 p-3 rounded-lg border border-zinc-800">
                                ...
                                <button onClick={() => removeDependent(dep.id)} aria-label={`Remove ${dep.name}`} className="text-zinc-600 hover:text-red-500"><Trash2 className="w-4 h-4" /></button>
                             </div>
                          ))}
                       </div>
                    </div>
