  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <div className="bg-zinc-900 rounded-2xl border border-zinc-800 w-full max-w-md p-6 shadow-2xl animate-fade-in">
         <h3 className="text-xl font-bold text-white mb-6">Criar Plano de Comissão</h3>
         <form onSubmit={handleCreatePlan} className="space-y-4">
            ...
            {newPlan.model === CompensationModel.CHAIR_RENTAL && (
               <div>
                  <label className="block text-xs font-medium text-zinc-400 mb-1.5">Taxa de Aluguel Fixa ($)</label>
            ...
            <div className="flex gap-3 pt-4 border-t border-zinc-800 mt-2">
              <button type="button" onClick={onClose} className="flex-1 py-3 text-zinc-500 hover:text-white font-medium">Cancelar</button>
              <button type="submit" className="flex-1 bg-amber-500 hover:bg-amber-400 text-zinc-950 font-bold py-3 rounded-xl">Salvar Plano</button>
            </div>
         </form>
      </div>
    </div>
