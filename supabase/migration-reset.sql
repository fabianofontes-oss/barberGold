-- =============================================
-- BARBERFLOW - RESET E MIGRAÇÃO COMPLETA
...
-- =============================================
-- DROP: Remove todas as tabelas existentes
-- =============================================
...
-- Módulo 7: Website
DROP TABLE IF EXISTS public.website_reviews CASCADE;
...
-- =============================================
-- Agora executa o schema completo
-- =============================================
...
CREATE TABLE public.appointments (
  ...
  price NUMERIC(10,2) NOT NULL,
  status TEXT DEFAULT 'SCHEDULED' CHECK (status IN ('SCHEDULED', 'CONFIRMED', 'IN_PROGRESS', 'COMPLETED', 'CANCELLED', 'NO_SHOW', 'BLOCKED')),
...
-- =============================================
-- MÓDULO 5: FINANCEIRO (parte 1 - cash_closures primeiro)
-- =============================================
...
-- =============================================
-- MÓDULO 5: VENDAS (agora pode referenciar cash_closures)
-- =============================================
...
CREATE TABLE public.expenses (
  ...
  category TEXT NOT NULL CHECK (category IN ('RENT', 'UTILITIES', 'SUPPLIES', 'EQUIPMENT', 'MARKETING', 'SALARIES', 'TAXES', 'MAINTENANCE', 'OTHER')),
...
CREATE TABLE public.website_config (
  ...
  custom_colors JSONB DEFAULT '{"primary": "#09090b", "secondary": "#18181b", "accent": "#f59e0b", "text": "#ffffff", "borderRadius": "1rem"}'::jsonb,
...
CREATE TABLE public.loyalty_config (
  ...
  points_currency_value NUMERIC(10,2) DEFAULT 1,
  tier_multipliers JSONB DEFAULT '{"BRONZE": 1, "SILVER": 1.25, "GOLD": 1.5, "PLATINUM": 2, "DIAMOND": 3}'::jsonb,
  tier_thresholds JSONB DEFAULT '{"BRONZE": 0, "SILVER": 500, "GOLD": 1500, "PLATINUM": 5000, "DIAMOND": 15000}'::jsonb,
...
CREATE TABLE public.referral_config (
  ...
  referrer_reward_value NUMERIC(10,2) DEFAULT 10,
...
CREATE TABLE public.notifications (
  ...
  title TEXT NOT NULL,
  body TEXT NOT NULL,
  type TEXT NOT NULL CHECK (type IN ('APPOINTMENT_REMINDER', 'APPOINTMENT_CONFIRMED', 'APPOINTMENT_CANCELLED', 'LOYALTY_POINTS_EARNED', 'LOYALTY_TIER_UP', 'BIRTHDAY', 'PROMOTION', 'SYSTEM')),
...
CREATE TABLE public.saas_plans (
  ...
  features JSONB DEFAULT '{"ONLINE_BOOKING": false, "LOYALTY": false, "ADVANCED_REPORTS": false, "MULTI_SHOP": false, "WEBSITE_PREMIUM": false, "COMMISSIONS": false, "BLIND_CASH_CLOSURE": false}'::jsonb,
  is_active BOOLEAN DEFAULT TRUE,
  sort_order INTEGER DEFAULT 0
);
...
-- =============================================
-- RLS: HABILITAR EM TODAS TABELAS
-- =============================================
...
CREATE POLICY "Users can view their tenant" ON public.tenants FOR SELECT
  USING (owner_id = auth.uid() OR id IN (SELECT tenant_id FROM public.profiles WHERE user_id = auth.uid()));
CREATE POLICY "Owners can update their tenant" ON public.tenants FOR UPDATE USING (owner_id = auth.uid());
CREATE POLICY "Anyone can create tenant" ON public.tenants FOR INSERT WITH CHECK (owner_id = auth.uid());

-- Profiles
CREATE POLICY "Users can view profiles of their tenant" ON public.profiles FOR SELECT
  USING (tenant_id = public.get_user_tenant_id() OR user_id = auth.uid());
CREATE POLICY "Owners can manage profiles" ON public.profiles FOR ALL USING (public.is_tenant_owner(tenant_id));
CREATE POLICY "Users can update their own profile" ON public.profiles FOR UPDATE USING (user_id = auth.uid());

-- Categories
CREATE POLICY "Users can view categories" ON public.categories FOR SELECT USING (tenant_id = public.get_user_tenant_id());
CREATE POLICY "Owners can manage categories" ON public.categories FOR ALL USING (public.is_tenant_owner(tenant_id));

-- Services
CREATE POLICY "Public can view active services" ON public.services FOR SELECT USING (is_active = TRUE);
CREATE POLICY "Owners can manage services" ON public.services FOR ALL USING (public.is_tenant_owner(tenant_id));

-- Products
CREATE POLICY "Public can view active products" ON public.products FOR SELECT USING (is_active = TRUE);
CREATE POLICY "Owners can manage products" ON public.products FOR ALL USING (public.is_tenant_owner(tenant_id));

-- Clients
CREATE POLICY "Staff can view clients" ON public.clients FOR SELECT USING (tenant_id = public.get_user_tenant_id());
CREATE POLICY "Staff can manage clients" ON public.clients FOR ALL USING (tenant_id = public.get_user_tenant_id());

-- Appointments
CREATE POLICY "Staff can view appointments" ON public.appointments FOR SELECT USING (tenant_id = public.get_user_tenant_id());
CREATE POLICY "Staff can manage appointments" ON public.appointments FOR ALL USING (tenant_id = public.get_user_tenant_id());

-- Sales
CREATE POLICY "Staff can view sales" ON public.sales FOR SELECT USING (tenant_id = public.get_user_tenant_id());
CREATE POLICY "Staff can create sales" ON public.sales FOR INSERT WITH CHECK (tenant_id = public.get_user_tenant_id());
CREATE POLICY "Owners can manage sales" ON public.sales FOR ALL USING (public.is_tenant_owner(tenant_id));

-- Sale Items
CREATE POLICY "Users can view sale_items" ON public.sale_items FOR SELECT
  USING (sale_id IN (SELECT id FROM public.sales WHERE tenant_id = public.get_user_tenant_id()));
CREATE POLICY "Users can insert sale_items" ON public.sale_items FOR INSERT
  WITH CHECK (sale_id IN (SELECT id FROM public.sales WHERE tenant_id = public.get_user_tenant_id()));

-- Cash Closures
CREATE POLICY "Staff can view cash closures" ON public.cash_closures FOR SELECT USING (tenant_id = public.get_user_tenant_id());
CREATE POLICY "Owners can manage cash closures" ON public.cash_closures FOR ALL USING (public.is_tenant_owner(tenant_id));

-- Expenses
CREATE POLICY "Owners can view expenses" ON public.expenses FOR SELECT USING (public.is_tenant_owner(tenant_id));
CREATE POLICY "Owners can manage expenses" ON public.expenses FOR ALL USING (public.is_tenant_owner(tenant_id));

-- Commissions
CREATE POLICY "Staff can view own commissions" ON public.commissions FOR SELECT
  USING (staff_id IN (SELECT id FROM public.profiles WHERE user_id = auth.uid()) OR public.is_tenant_owner(tenant_id));
CREATE POLICY "Owners can manage commissions" ON public.commissions FOR ALL USING (public.is_tenant_owner(tenant_id));

-- Website Config
CREATE POLICY "Public can view website config" ON public.website_config FOR SELECT USING (TRUE);
CREATE POLICY "Owners can manage website config" ON public.website_config FOR ALL USING (public.is_tenant_owner(tenant_id));

-- Website Reviews
CREATE POLICY "Public can view published reviews" ON public.website_reviews FOR SELECT USING (is_published = TRUE);
CREATE POLICY "Owners can manage reviews" ON public.website_reviews FOR ALL USING (public.is_tenant_owner(tenant_id));

-- Loyalty Config
CREATE POLICY "Staff can view loyalty config" ON public.loyalty_config FOR SELECT USING (tenant_id = public.get_user_tenant_id());
CREATE POLICY "Owners can manage loyalty config" ON public.loyalty_config FOR ALL USING (public.is_tenant_owner(tenant_id));

-- Loyalty Transactions
CREATE POLICY "Staff can view loyalty transactions" ON public.loyalty_transactions FOR SELECT USING (tenant_id = public.get_user_tenant_id());
CREATE POLICY "Staff can create loyalty transactions" ON public.loyalty_transactions FOR INSERT WITH CHECK (tenant_id = public.get_user_tenant_id());

-- Loyalty Rewards
CREATE POLICY "Staff can view loyalty rewards" ON public.loyalty_rewards FOR SELECT USING (tenant_id = public.get_user_tenant_id());
CREATE POLICY "Owners can manage loyalty rewards" ON public.loyalty_rewards FOR ALL USING (public.is_tenant_owner(tenant_id));

-- Referral Config
CREATE POLICY "Staff can view referral config" ON public.referral_config FOR SELECT USING (tenant_id = public.get_user_tenant_id());
CREATE POLICY "Owners can manage referral config" ON public.referral_config FOR ALL USING (public.is_tenant_owner(tenant_id));

-- Referral Links
CREATE POLICY "Staff can view referral links" ON public.referral_links FOR SELECT USING (tenant_id = public.get_user_tenant_id());
CREATE POLICY "Staff can create referral links" ON public.referral_links FOR INSERT WITH CHECK (tenant_id = public.get_user_tenant_id());

-- Referral Conversions
CREATE POLICY "Staff can view referral conversions" ON public.referral_conversions FOR SELECT USING (tenant_id = public.get_user_tenant_id());

-- Tips
CREATE POLICY "Staff can view tips" ON public.tips FOR SELECT USING (tenant_id = public.get_user_tenant_id());
CREATE POLICY "Staff can create tips" ON public.tips FOR INSERT WITH CHECK (tenant_id = public.get_user_tenant_id());
...
-- SaaS Invoices
CREATE POLICY "Owners can view invoices" ON public.saas_invoices FOR SELECT USING (public.is_tenant_owner(tenant_id));

-- Support Tickets
CREATE POLICY "Owners can view support tickets" ON public.support_tickets FOR SELECT USING (public.is_tenant_owner(tenant_id));
CREATE POLICY "Owners can create support tickets" ON public.support_tickets FOR INSERT WITH CHECK (public.is_tenant_owner(tenant_id));
CREATE POLICY "Owners can update support tickets" ON public.support_tickets FOR UPDATE USING (public.is_tenant_owner(tenant_id));

-- Support Messages
CREATE POLICY "Users can view messages" ON public.support_messages FOR SELECT
  USING (ticket_id IN (SELECT id FROM public.support_tickets WHERE public.is_tenant_owner(tenant_id)));
CREATE POLICY "Users can create messages" ON public.support_messages FOR INSERT
  WITH CHECK (ticket_id IN (SELECT id FROM public.support_tickets WHERE public.is_tenant_owner(tenant_id)));
